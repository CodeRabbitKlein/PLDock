...
def generate_conformer(mol):
    ...
    try:
        ...
        get_logger().warning(f'Molecule sanitization failed: {e}. Proceeding with original structure.')
    ...
    if id == -1:
        ...
        if id == -1:
            get_logger().warning('Random coordinate embedding also failed. Using basic embedding without optimization.')
            # 最后的备选方案：使用最基本的嵌入
            ps = AllChem.ETKDG()
            ps.useRandomCoords = True
...
'''
增强的构象生成策略:
    多层次构象生成：首先使用ETKDGv2，失败时使用随机坐标，最后使用基础ETKDG
    分子预处理：在构象生成前进行分子清理和立体化学检查
    智能失败处理：即使所有构象生成方法都失败，仍然返回基本的3D坐标

多种优化方法备选
    MMFF优化：首选方法，适用于大多数有机分子
    UFF优化：备选方法，适用于MMFF失败的情况（特别是含金属的体系）
    基本清理：最后的备选，至少进行基本的结构整理
'''

def _optimize_molecule(mol):
    """尝试多种优化方法，处理MMFF优化失败的情况"""
    
    # 方法1: MMFF优化
    try:
        mmff_result = AllChem.MMFFOptimizeMolecule(mol, confId=0)
        if mmff_result == 0:
            get_logger().debug('MMFF optimization successful.')
            return True
        else:
            get_logger().warning(f'MMFF optimization returned non-zero status: {mmff_result}. Trying alternative methods.')
    except Exception as e:
        get_logger().warning(f'MMFF optimization failed with exception: {e}. Trying alternative methods.')
    
    # 方法2: UFF优化（作为备选）
    try:
        uff_result = AllChem.UFFOptimizeMolecule(mol, confId=0)
        if uff_result == 0:
            get_logger().info('UFF optimization successful as fallback.')
            return True
        else:
            get_logger().warning(f'UFF optimization also returned non-zero status: {uff_result}.')
    except Exception as e:
        get_logger().warning(f'UFF optimization failed: {e}.')
    
    # 方法3: 使用简单的力场清理
    try:
        # 尝试基本的清理，不强制优化
        AllChem.Cleanup(mol, confId=0)
        get_logger().info('Basic cleanup applied as optimization fallback.')
        return True
    except Exception as e:
        get_logger().warning(f'Basic cleanup failed: {e}. Using unoptimized conformer.')
    
    # 如果所有优化都失败，仍然返回True，因为至少有基本的3D坐标
    get_logger().info('Using unoptimized conformer.')
    return True
